...
def compute_gradient_of_variables(output_tensor: "Tensor", out_grad: "Tensor"):
    ...
    for node in reverse_topo_order:
        ...
        if not hasattr(node, 'grad') or node.grad is None:
            # Initialize grad to zero with the same shape as the first adjoint
            if node in node_to_output_grads_list:
                first_adjoint = node_to_output_grads_list[node][0]
                node.grad = init.zeros_like(first_adjoint, requires_grad=False)
        
        # Sum all adjoints into grad (for both leaf and non-leaf nodes)
        if node in node_to_output_grads_list:
            for adjoint in node_to_output_grads_list[node]:
                node.grad = node.grad + adjoint
        
        # For non-leaf nodes, compute gradients for inputs
        if node.op is not None:
            all_partial_adjoints = node.op.gradient_as_tuple(node.grad, node)
            for input, partial_adjoint in zip(node.inputs, all_partial_adjoints):
